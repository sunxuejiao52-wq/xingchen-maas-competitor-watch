# Xingchen MaaS Competitor Watch

星辰 MaaS 竞品动态监测看板，用于跟踪 MaaS 竞品的模型更新、功能变化、宣传动作、公众号和新闻线索。

## 在线发布建议

推荐使用 GitHub + Cloudflare Pages：

- GitHub Actions 每天北京时间 01:00 自动运行数据刷新脚本。
- Cloudflare Pages 连接本仓库后，发布 `outputs/cloudflare-pages`。
- 页面打开时会自动读取最新的 `xingchen-maas-competitor-data.js`，时间筛选会跟随最新数据日期更新。
- 页面上的“数据刷新”按钮会调用 Cloudflare Pages Function 触发 GitHub Actions，从配置来源采集当天数据。

Cloudflare Pages 设置：

```text
Production branch: main
Build command: npm run prepare:cloudflare
Build output directory: outputs/cloudflare-pages
Functions directory: functions
```

## Cloudflare 环境变量

若要让“数据刷新”按钮真正触发采集，需要在 Cloudflare Pages 项目里配置：

```text
GITHUB_ACTIONS_TOKEN=<GitHub Personal Access Token>
```

Token 建议使用 classic token，并勾选 `repo` 和 `workflow` 权限。不要把 token 写进网页或仓库文件里。

## Netlify 兼容说明

仓库仍保留 Netlify 配置，便于之后回退：

- GitHub Actions 每天北京时间 01:00 自动运行数据刷新脚本。
- Netlify 连接本仓库后，读取 `netlify.toml`，发布 `outputs/netlify-upload`。
- 页面打开时会自动读取最新的 `xingchen-maas-competitor-data.js`，时间筛选会跟随最新数据日期更新。
- 页面上的“数据刷新”按钮会调用 Netlify Function 触发 GitHub Actions，从配置来源采集当天数据。

## Netlify 环境变量

若要让“数据刷新”按钮真正触发采集，需要在 Netlify 站点里配置：

```text
GITHUB_ACTIONS_TOKEN=<GitHub Personal Access Token>
```

Token 建议使用 classic token，并勾选 `repo` 和 `workflow` 权限。不要把 token 写进网页或仓库文件里。

## 本地命令

```bash
npm run refresh:daily:dry -- --target-date 2026-07-14
npm run prepare:cloudflare
npm run prepare:netlify
```

## 关键文件

- `index.html`：主页面
- `xingchen-maas-competitor-data.js`：网页读取的数据文件
- `scripts/daily-competitor-refresh.mjs`：每日采集与数据更新脚本
- `config/competitor-monitor-sources.json`：竞品监测来源和关键词
- `.github/workflows/daily-competitor-refresh.yml`：GitHub Actions 定时任务
- `functions/api/trigger-refresh.js`：Cloudflare Pages 手动触发数据刷新入口
- `wrangler.toml`：Cloudflare Pages 构建配置
- `netlify.toml`：Netlify 发布配置
- `netlify/functions/trigger-refresh.js`：页面手动触发数据刷新时调用的后台入口
